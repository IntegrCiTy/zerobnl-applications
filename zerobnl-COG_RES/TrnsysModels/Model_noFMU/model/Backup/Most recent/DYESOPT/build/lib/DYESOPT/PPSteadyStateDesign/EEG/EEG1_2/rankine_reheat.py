...
def rankine_reheat(PC_mdot):
    ...
    #     PC_COND.P = PROP(2)
        ...
        if PC_DoubleCond   == 0:
           ...
           PC_DH_T_return  = PC_DH_T_supply - (PC_Heat_demand*1000/(PC_DH_massflow*4.186))
           APPROACH_COND1  = (PC_DH_T_supply - PC_DH_T_return)*PC_Double_approach
           PC_COND1_Tcout  = PC_DH_T_return + APPROACH_COND1 + PC_dT_wtr_out_condensing
           ...
           # used.
           PC_COND2_Tcout  = PC_dT_wtr_out_condensing + PC_DH_T_supply
    ...
    if PC_DoubleCond == 0:                                                                                 # Rankine cycle simple with IPT
        ...
        PC_st[LPTout_n_st-1,1] = PC_COND1_P*(1+PC_COND_Pdrop)             #pressure at the LPT outlet and at the previous LP-stage outlet

        PC_st[LPTout_n_st-2,1] = PC_COND2_P*(1+PC_COND2_Pdrop)

        PC_st[LPTout_n_st-3,1] = PC_COND2_P*(1+PC_COND2_Pdrop)

        PC_st[LPTout_n_st-3-PC_DeaLP,1] = PC_COND2_P*(1+PC_COND2_Pdrop)

        PC_st[(LPTout_n_st+PC_n_hotPH + 1 + PC_n_coldPH + PC_DeaLP + PC_DoubleCond)-1,1]= PC_COND2_P*(1+PC_COND2_Pdrop)



    PC_st[HPTin_n_st-1,1]     = PC_PinHPT

    PC_st[IPTin_n_st-1,1]     = PC_PinIPT

    if PC_DeaLocatHP == 1:
        PC_st[LPTin_n_st-1,1] = PC_PinIPT                                   # if the Dea is located before the RH, the IPT stage doesn't really exist
    ...
    if design_TinHPTset == 1:
        ...
        HPT_inlet_point = IAPWS97(P=PC_st[(HPTin_n_st)-1,1]/10, T=PC_st[(HPTin_n_st)-1,0]+273.15)
    ...
    SHeatInlet_point= IAPWS97(P=PC_st[(HPTin_n_st-PC_BackPrValve-1)-1,1]/10, x= 1)
    ...
    if PC_BoilerMode >0:                                         # If the economizer is included, the EVA inlet conditions need still to be defined
        ...
        EvapInlet_point= IAPWS97(P=PC_st[(HPTin_n_st-PC_BackPrValve-2)-1,1]/10, x= 0)
    ...
    # BackPressure Condenser outlet
    # if PC_BackPrValve == 1
    # PC_st((LPTout_n_st + PC_n_hotPH + 1 + PC_n_coldPH + PC_DeaLP + ...
    #             PC_DoubleCond + 2*(PC_n_hotPH + PC_n_coldPH) + 4*PC_BackPrValve + PC_LPextr),5) = 0; #42
    #
    #
    # PC_st((LPTout_n_st + PC_n_hotPH + 1 + PC_n_coldPH + PC_DeaLP + ...
    #             PC_DoubleCond + 2*(PC_n_hotPH + PC_n_coldPH) + 4*PC_BackPrValve + PC_LPextr),:)= ...
    #             sat_state(PC_st((LPTout_n_st + PC_n_hotPH + 1 + PC_n_coldPH + PC_DeaLP + ...
    #             PC_DoubleCond + 2*(PC_n_hotPH + PC_n_coldPH) + 4*PC_BackPrValve + PC_LPextr),2),...
    #             PC_st((LPTout_n_st + PC_n_hotPH + 1 + PC_n_coldPH + PC_DeaLP + ...
    #             PC_DoubleCond + 2*(PC_n_hotPH + PC_n_coldPH) + 4*PC_BackPrValve + PC_LPextr),5),...
    #             PC_st((LPTout_n_st + PC_n_hotPH + 1 + PC_n_coldPH + PC_DeaLP + ...
    #             PC_DoubleCond + 2*(PC_n_hotPH + PC_n_coldPH) + 4*PC_BackPrValve + PC_LPextr),8));
    # end
    ...
    if design_HTFloop_set == 1:
        [PC_EV_EFF,    PC_EV_UA]     = evaporator(HTF[3,:],PC_st[HPTin_n_st-2-1,:],HTF[4,:])                     #You take inlet and outlet conditions of EV (HTF side) and inlet conditions of EV (steam side)
        [PC_ECO_EFF,   PC_ECO_UA]    = economizer(PC_st[HPTin_n_st-3-1,:],HTF[4,:],PC_st[HPTin_n_st-2-1,:])      #You take inlet and outlet conditions of EC (steam side) and inlet conditions of EC(HTF side)
        [PC_SH_EFF,    PC_SH_UA]     = economizer(PC_st[HPTin_n_st-1-1,:],HTF[1,:],PC_st[HPTin_n_st-1,:])        #You take inlet and outlet conditions of SH (steam side) and inlet conditions of SH(HTF side)
    ...
    return [PC_COND_Area, PC_COND2_Area, PC_COND1_Area, PC_W_HPT, PC_W_LPT, PC_Wparas, PC_W, APPROACH_COND1, PC_COND2_UA_DELTA, PC_COND_air_FR, PC_COND_fan_Power, PC_COND_Tcout, PC_COND_UA,PC_coldPH, PC_hotPH, PC_coldSC, PC_hotSC, PC_PUMP_W2, PC_PUMP_W_DeaLP, PC_PumpBefMix_Power,PC_ECO_Q, PC_EV_Q ,PC_SH_Q,PC_Wgross_el,PC_W_el, PC_CONDs_Q, PC_st,PC_keyst_DEAin_n_st, PC_keyst_PUMP2out_n_st, PC_keyst_HPTin_n_st, PC_keyst_IPTin_n_st, PC_keyst_LPTin_n_st, PC_keyst_LPTout_n_st, PC_LPT_ETA, PC_IPT_ETA, PC_HPT_ETA]
